	...
	echo:		Do not display the user's response if this flag is FALSE. */
{
	...
	/* API "and" chain. If any test or system call fails, the
		...
		//ENABLE_PROCESSED_INPUT: control key ctrl-cn is handled by the system and not stored in the input buffer. So if we enter ctrl-c the os terminate the process.
